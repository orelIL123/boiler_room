...
export default function CommunityScreen() {
  ...
  return (
    <View style={{ flex:1, backgroundColor: BG, paddingTop: Platform.select({ ios: 48, android: 24 }) }}>
      {/* Header with back button */}
      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 20, marginTop: 8, paddingHorizontal: 16 }}>
        <Pressable
      ...
      >
        {eventStartingToday && (
          <View style={{ marginBottom:16, padding:14, borderRadius:12, backgroundColor:'rgba(212,175,55,0.12)', borderWidth:1, borderColor:'rgba(212,175,55,0.35)', flexDirection:'row', alignItems:'center', gap:10 }}>
            ...
            >
              <Text style={{ color:'#000', fontFamily:'Heebo_700Bold' }}>הוסף ל יומן</Text>
            </Pressable>
          </View>
        )}
        ...
        {events && events.length > 0 ? (
          events.map((item) => (
            <EventRow key={item.id} item={item} onAddToCalendar={addToCalendar} />
          ))
        ) : (
          <Text style={{ color: '#9CA3AF', fontFamily: 'Heebo_400Regular', textAlign: 'right', marginTop: 8 }}>
            אין אירועים קרובים
          </Text>
        )}
  ...
  return (
    <View style={{ padding:14, borderRadius:12, backgroundColor:'rgba(255,255,255,0.06)', marginBottom:10 }}>
      ...
      >
        ...
        <Text style={{ color: GOLD, fontFamily: 'Heebo_600SemiBold', fontSize: 14 }}>הוסף ליומן</Text>
      </Pressable>
    </View>
  )
}
